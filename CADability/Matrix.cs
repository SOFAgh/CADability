...
namespace CADability.LinearAlgebra
{
    ...
    public class Matrix : ICloneable
    {
        ...
        public Matrix(double[] vals, bool makeColumnMatrix)
        {
            if (makeColumnMatrix)
            {
                A = new double[vals.Length, 1];
        ...
        public Matrix(params GeoVector[] v1)
        {
            A = new double[v1.Length, 3];
        ...
        public Matrix(params GeoVector2D[] v1)
        {
            A = new double[v1.Length, 2];
            for (int i = 0; i < v1.Length; ++i)
            {
                A[i, 0] = v1[i].x;
                A[i, 1] = v1[i].y;
        ...
        public virtual Matrix SaveSolve(Matrix B)
        {
            ...
            {
                QRDecomposition rq = new QRDecomposition(this);
                if (rq.FullRank)
                {
                    return rq.Solve(B);
                }
        ...
        public virtual Matrix SaveSolveTranspose(Matrix B)
        {
            return Transpose(this).SaveSolve(Transpose(B));
        }
        ...
        public virtual Matrix SaveInverse()
        {
            return SaveSolve(Identity(rowCount, rowCount));
        }
        ...
        {
            Matrix m1 = new Matrix(1, d1.Length);
        ...
        public static Matrix operator *(Matrix m1, double[] d2)
        {
            Matrix m2 = new Matrix(d2.Length, 1);
        ...
        internal static void SwapColumns(Matrix m, int j, int k)
        {
            for (int i = 0; i < m.RowCount; i++)
            {
                double tmp = m[i, j];

...
namespace CADability.Forms
{
    ...
    public class PaintToOpenGL : IPaintTo3D
    {
        ...
        public void Init(IntPtr deviceContext, int width, int height, bool toBitmap)
        {
            ...
            CheckError();
            Gl.glGetIntegerv(Gl.GL_MAX_MODELVIEW_STACK_DEPTH, out int stackDepth);
            ...
            if (toDelete.Count > 0)
            {
                lock (toDelete)
                {
                    for (int i = 0; i < toDelete.Count; ++i)
                    {
                        ...
                        if (!openLists.ContainsKey(toDelete[i])) { }
#endif
                        openLists.Remove(toDelete[i]);
                        try
                        {
#if DEBUG
                            int ok = Gl.glIsList(toDelete[i]);
                            if (ok == 0) { }
#endif
